...
import {
  ...
  Clock, ClipboardList, Star, ChevronLeft, X, Calendar, Users, Search, Bell, MapPin, Globe, Cake, ChevronUp, ChevronDown, ChevronRight, Wand
} from 'lucide-react';
import { StorageService } from './services/storage';

// --- Global Constants ---
// Colors are now handled by Tailwind config (primary, secondary)
const SERVICE_AREAS = [
...
const ModalContainer = ({ children, onClose }) => (
  <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
    <div className="w-full max-w-sm bg-white rounded-2xl shadow-2xl relative transition-all duration-300 transform scale-100 opacity-100">
      {/* Close button removed from here to avoid overlap, moved to children */}
      {children}
    </div>
  </div>
);
...
const BookingPage = ({ goToPage, selectedService, showNotification }) => {
  const todayISO = new Date().toISOString().split('T')[0];
  const [date, setDate] = useState(todayISO);
  const [time, setTime] = useState('10:00');
  const [area, setArea] = useState(SERVICE_AREAS[0]);
  const [address, setAddress] = useState('Plot 45, Street 10, DHA Phase 2');
  const [loading, setLoading] = useState(false);
...
const BookingsPage = ({ goToPage, showNotification }) => {
  ...
  const handleCancel = (bookingId) => {
    ...
    setBookings(updated);
    showNotification(`Booking #${bookingId} has been cancelled.`, 'info');
  };
...
const ProfilePage = ({ goToPage, onLogout }) => {
    ...
    <div className="flex flex-col flex-1 pb-16 overflow-y-auto bg-gray-50">
      ...
      <div className="p-6 max-w-4xl mx-auto w-full">
        ...
        <div className="p-4 bg-white rounded-xl shadow-lg mb-6">
          ...
          <ProfileItem icon={UserCircle} label="Full Name" value={user.name} />
          <ProfileItem icon={Bell} label="Email" value={user.email} />
          <ProfileItem icon={Clock} label="Phone" value={user.phone} />
          <ProfileItem icon={MapPin} label="Default Address" value={user.address.substring(0, 30) + '...'} />
        </div>
        <div className="p-4 bg-white rounded-xl shadow-lg mb-6">
...
const App = () => {
  ...
  const renderPage = () => {
    if (!isLoggedIn) {
      return <LoginPage onLoginSuccess={handleLoginSuccess} onRegisterPress={() => showNotification('Registration not implemented yet!', 'info')} />;
    }
    switch (currentPage) {
      case 'Services': return <ServicesPage goToPage={goToPage} setSelectedService={setSelectedService} />;
      case 'Booking': return <BookingPage goToPage={goToPage} selectedService={selectedService} showNotification={showNotification} />;
      case 'Bookings': return <BookingsPage goToPage={goToPage} showNotification={showNotification} />;
      case 'Workers': return <WorkersPage goToPage={goToPage} />;
      case 'Profile': return <ProfilePage goToPage={goToPage} onLogout={handleLogout} />;
      case 'Notifications': return <NotificationsPage goToPage={goToPage} />;
      default: return <ServicesPage goToPage={goToPage} setSelectedService={setSelectedService} />;
    }
  };
  ...
  return (
    <div className="min-h-screen flex flex-col antialiased bg-gray-50 text-gray-800 max-w-lg mx-auto shadow-xl">
      {renderPage()}
      {isLoggedIn && currentPage !== 'Login' && <BottomNavBar currentPage={currentPage} goToPage={goToPage} />}
